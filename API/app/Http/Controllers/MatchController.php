...
class MatchController extends Controller
{
    ...
    public function createTeam(Request $request)
    {
        ...
        foreach ($lists as $key => $value) {
            ...
            if (@$value->player->role == 'batsman') {
                ...
                $data['bat'][$batindex]['profile'] = $profile;
                $batindex++;
                ...
                $data['bowl'][$bowlindex]['profile'] = $profile;
                $bowlindex++;
                ...
                $data['ar'][$arindex]['profile'] = $profile;
                $arindex++;
                ...
                $data['wk'][$wkindex]['profile'] = $profile;
                $wkindex++;
            }
        }
    ...
    public function getplayerdetail(Request $request)
    {
        ...
        // dd($player_details->schedule->short_name);
    ...
    {
        $match_details = FantasyTeam::where('match_key', 'bplt20_2020_g31')
            ->where('contest_id', '5def40b3d59ef822b1ba756f')
            ->where('paid_status', 1)
            ->get();

        // dd($match_details);

        // $match_details->cancelled = 1;
        // $match_details->save();

        $details = FantasyTeam::get();

        $schedule_details = Schedule::where('status', 'completed')->get();

        // $user_details = User::where('_id','5deb7ff06dfec71b0b1d5888')->first();
        // $user_details->wallet = ['deposit' => 3000,
        //     'total' => 5000,
        //     'bonus' => 1000,
        //     'currency' => '₹',
        //     'winnings' => 1000,
