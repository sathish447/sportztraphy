...
class ContestController extends Controller
{
    public function Contest_list(Request $request)
    {
        ...
        foreach ($details as $cat => $value) {
            ...
            $team_create_check = FantasyTeam::where('match_key', $request->id) 
                ->where('user_id', auth()->user()->_id)
                ->first();
            ...
            $pick_team_create_check = HeroTeam::where('match_key', $request->id) 
                ->where('user_id', auth()->user()->_id)
                ->first();

            $pick_team_create_count = 0;
            if (is_object($pick_team_create_check)) {
                $pick_team_create_count = $pick_team_create_check->count();
                $pick_create_team_status = 1;
                $paid_status = $pick_team_create_check->paid_status;
            } else {
                $pick_create_team_status = 0;
                $paid_status = 0;
            }
            ...
            $value->pick_create_team_status = $pick_create_team_status;
            ...
            $con_details['pick_create_team_status'] = $value->pick_create_team_status;
        ...
        $response = array(
                        'contestdetails' => $temp,
                        'team_create_count' => count($team_details),
  ...
  public function TournmentContest_list(Request $request)
    {
        ...
        $details = Contest::where('created_by', 'admin')
            ->where('status', 1)
            ->where('contest_type','pick')
            ->orderBy('_id', 'DESC')
            ->get();


        $team_details = TournmentTeam::select('_id')
        ...
        foreach ($details as $cat => $value) {
            ...
            $listcount = TournmentTeam::where('contest_id', $value->_id)
                ->where('session_name', $request->session_name)
            ...
            $team_create_check = TournmentTeam::where('session_name', $request->session_name) 
                ->where('format', $request->format) 
                ->where('user_id', auth()->user()->_id)
                ->first();
            ...
            $joinedlistcount = TournmentTeam::where('contest_id', $value->_id)
                ->where('session_name', $request->id)
            // ->where('user_id',auth()->user()->_id)
                ->where('paid_status', 1)
                ->count();
    ...
    public function joincontest(Request $request)
    {
        ...
        if ($user_details->wallet['total'] != 0 && $user_details->wallet['deposit'] >= $request->entryfee || $user_details->wallet['winnings'] >= $request->entryfee) {
            ...
            if (is_object($listcount)) {
                ...
                $yourData = ['status' => true, 'create_status' => false, 'message' => 'First create your team.'];
                return response()->json($yourData, 200, ['Content-Type' => 'application/json;charset=UTF-8', 'Charset' => 'utf-8']);
            }
            ...
            $yourData = ['status' => false, 'response' => '',  'message' => 'Not enough balance.Please deposit and try.'];
    ...
    public function jointournamentcontest(Request $request)
    {
         $validator = Validator::make($request->all(), [
            'entryfee' => 'required',
            'session_name' => 'required',
        ...
        if ($user_details->wallet['total'] != 0 && $user_details->wallet['deposit'] >= $request->entryfee || $user_details->wallet['winnings'] >= $request->entryfee) {
            ...
            if (isset($request->teamid)) {
                ...
                $listcount = TournmentTeam::where('session_name', $request['session_name'])
                    ->where('_id', $request['teamid'])
                    ->where('user_id', auth()->user()->_id)
                    ->first();
            ...
            if (is_object($listcount)) {
                ...
                if ($listcount->paid_status == 1) {
                    ...
                    $response = array('create_status' => false);

                    $yourData = ['status' => false, 'response' => $response, 'message' => 'You already joined a contest.please create another team and join again'];
                ...
                if (is_object($cat_details)) {
                    ...
                    $joinedlistcount = TournmentTeam::where('contest_id', $request->cat_id)
                        ->where('session_name', $request->session_name)
                        ->where('paid_status', 1)
                        ->count();
                    ...
                    if ($joinedlistcount >= $cat_details->contest_size) {
                        ...
                        $yourData = ['status' => false, 'response' => $response, 'message' => 'Contest Full.'];
                ...
                $response = array('create_status' => false);
                $yourData = ['status' => false, 'response' => $response, 'message' => 'First create your team.'];
