...
<!-- Brand -->
<a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block" href="#"> BONUS LIST</a>
<!-- Form -->
	 <form class="navbar-search navbar-search-dark form-inline mr-3 d-none d-md-flex ml-lg-auto" action="{{ url('/admin/bonus/search') }}" method="post" autocomplete="off">
	{{ csrf_field() }}  
...
<a href="{{ url('admin/create_view') }}" class="btn btn-sm btn-primary"><i class="fa fa-arrow-right"></i> Create Bonus</a> 
</div>
</div>
...
<th scope="col">Contest Joined Bonus</th>
...
<td>  <div class="cwidth-200box wraptextbg">{{ $con->referalbonus }}</div></td>
<td>{{ $con->joinedbonus }}</td>
<td>{{ $con->type }}</td>

<td>

 <a href="{{ url('admin/edit_view/'.$con->_id) }}" class="btn btn-sm btn-info" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fas fa-pencil-alt"></i></a>       
