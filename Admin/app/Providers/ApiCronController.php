...
class ApiCronController extends Controller
{
    ...
    {
        $url = "https://rest.cricketapi.com/rest/v2/schedule/?access_token=" . $this->access_token . "&date=2020-01";
                        ...
                        foreach ($days as $dy):
                            if (count($dy->matches) > 0):
                                foreach ($dy->matches as $match):
                                    if ($match->key = "mzslt20_2019_final" || "qtrlt10_2019_final") {
                                        $dbmatch = Schedule::where('key', $match->key)->first();
                                        if (is_object($dbmatch)) {
                                            ...
                                            $dbmatch->msg_info = $match->msgs->info;
                                            $dbmatch->msg_completed = $match->msgs->completed;
                                            $dbmatch->venue = $match->venue;
